...
import com.crazedout.cosplay.pojo.POJOAudio;
...
public class CosMapFactory {
    ...
    public static void exportEditable(MapPanel panel, File file, boolean exportImages) throws Exception {
        Map map = panel.map;
        ...
        for(Sprite s:map.getSprites()){
            if(s instanceof DefaultUserSprite) {
                ...
                w.setContinuousSoundId(us.getContinuousSoundId());
                w.setEffectsSoundId(us.getEffectSoundId());
                sprites.add(w);
            }else{
    ...
    public static void export(File tmpDir, File fileToSave, Map map, boolean exportImages) throws Exception {
        ...
        List<DefaultSpriteWrapper> sprites = new ArrayList<>();
        for(Sprite s:map.getSprites()){
            exportSoundFile(tmpDir, s.getContinuousSoundId());
            exportSoundFile(tmpDir, s.getEffectSoundId());
            if(s instanceof DefaultUserSprite) {
                DefaultUserSprite us = (DefaultUserSprite) s;
                DefaultSpriteWrapper w = new DefaultSpriteWrapper(s.getId(), s.getClass().getName(),
                        us.getImageMap(), s.getSpeed(), s.getStartTile());
                w.setContinuousSoundId(s.getContinuousSoundId());
                w.setEffectsSoundId(s.getEffectSoundId());
                sprites.add(w);
                if (exportImages) {
        ...
        if(exportImages) {
            for (Tile t : map.getGrid()) {
                if(t.getImage()!=null) {
                    if(t.getImage() instanceof SubImageBitmap){
                        FileOutputStream fos = new FileOutputStream((tmpDir.getAbsolutePath())+File.separatorChar +
                                ((SubImageBitmap)t.getImage()).fileName);
                         ((SubImageBitmap)t.getImage()).saveImage(fos,"png");
                    }else {
                        exportFile(tmpDir, t.getImage());
                    }
                    if(t.getItem()!=null){
                        exportSoundFile(tmpDir,t.getItem().getSoundId());
                    }
                }
            }
        }

        for(Integer id:((CosMap)map).sounds.values()){
            exportSoundFile(tmpDir,id);
        }
        exportSoundFile(tmpDir,GamePlay.getAudio().findId("opengate.au"));
        ...
        protected static void exportSoundFile(File outDir, int soundId) {
            CosAudio a = (CosAudio)GamePlay.getAudio();
            if(soundId<1 || soundId>a.getNumSounds()-1) return;

                String file = System.getProperty("user.home") + File.separatorChar +
                    "cosplay" + File.separatorChar + "sound" + File.separatorChar +
                    a.getSoundFile(soundId);
            try {
                InputStream in = new FileInputStream(file);
                FileOutputStream fos = new FileOutputStream(outDir.getAbsolutePath() +
                        File.separatorChar + a.getSoundFile(soundId));
    ...
    public static void importEditableMap(String file, MapPanel panel, Class c){
        ...
        try {
            ...
            ex.printStackTrace();
            CosPlay.showErrorDialog("Error CosMapFactory",ex.getMessage());
        }
    }
