...
public class KeyControls extends KeyAdapter implements Serializable {
    ...
    public void keyPressed(KeyEvent e){
        ...
        if(e.getKeyCode()==KeyEvent.VK_F1){
            GamePlay.getGamePlay().setGameState(GamePlay.GameState.GAME_ON);
            System.out.println("F1");
        }
        if(e.getKeyCode()==KeyEvent.VK_F2){
            GamePlay.getGamePlay().setGameState(GamePlay.GameState.GAME_OVER);
        }
        if(e.getKeyCode()==KeyEvent.VK_F3){
            map.setOffset(new Offset(0,0));
            panel.repaint();
        }
