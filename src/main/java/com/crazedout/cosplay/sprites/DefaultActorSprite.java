...
public class DefaultActorSprite extends Sprite implements GamePlayListener{
    ...
    public void hit(Object obj){
        hitCounter=0;
        if(getEffectSoundId()>0){
            GamePlay.getAudio().playSound(getEffectSoundId());
        }
        if(getContinuousSoundId()>0){
            GamePlay.getAudio().stopSound(getContinuousSoundId());
        }
    ...
    public void gamePlayChanged(GamePlay gp) {
            switch (gp.getGameState()) {
                case GAME_ON:
                    GamePlay.getAudio().playSound(getContinuousSoundId(), true);
                    break;
                case GAME_OVER:
                case MARQUEE:
                    GamePlay.getAudio().stopSound(getContinuousSoundId());
